...
stopuser = {}
token = '6472151902:AAEfR_J2LMCppI3n9ur9J7GR9k-29lDcTOg'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/minux_rida")
			keyboard.add(contact_button)
			random_number = random.randint(33, 82)
			photo_url = f'https://t.me/minux_rida/{random_number}'
			bot.send_photo(chat_id=message.chat.id, photo=photo_url, caption=f'''<b>مرحبا {name}
بك في بوت فحص خاص بي مينكس رضى
لي عمل فحص مجاني ارسل امر 
/cmds {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/minux_rida")
		keyboard.add(contact_button)
...
𝘽𝙤𝙩 𝘽𝙮 @minux_rida''', reply_markup=mes)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='minux-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
